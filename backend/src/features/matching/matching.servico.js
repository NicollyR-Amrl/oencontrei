...
function calcularScore(itemPerdido, itemEncontrado) {
  ...
  if (diffDias <= 1) scoreTotal += 0.1;
  else if (diffDias <= 3) scoreTotal += 0.05;
  else if (diffDias <= 7) scoreTotal += 0.02;
...
async function buscarMatches(itemPerdidoId) {
  const itemPerdido = await prisma.item.findUnique({ where: { id: itemPerdidoId } });
  ...
  const itensEncontrados = await prisma.item.findMany({
    where: { tipo: 'ENCONTRADO', status: 'ATIVO' },
  ...
  for (const resultado of resultados) {
    const matchExistente = await prisma.match.findFirst({
      where: { itemPerdidoId, itemEncontradoId: resultado.itemEncontrado.id }
    });

    if (matchExistente) {
      await prisma.match.update({ where: { id: matchExistente.id }, data: { score: resultado.score } });
    } else {
      await prisma.match.create({
        data: { itemPerdidoId, itemEncontradoId: resultado.itemEncontrado.id, score: resultado.score }
      });
    }
  }
...
async function buscarMatchesParaEncontrado(itemEncontradoId) {
  const itemEncontrado = await prisma.item.findUnique({ where: { id: itemEncontradoId } });
  ...
  const itensPerdidos = await prisma.item.findMany({
    where: { tipo: 'PERDIDO', status: 'ATIVO' },
    include: { usuario: { select: { id: true, nome: true, avatar: true } } }
  });

// Feature: Matching — Controlador

const prisma = require('../../shared/prisma');
const { buscarMatches, buscarMatchesParaEncontrado } = require('./matching.servico');
const { criarNotificacao, notificarMatch } = require('../notificacoes/notificacoes.servico');

/**
 * Buscar matches para um item
...
const obterMatches = async (req, res) => {
  try {
    ...
    if (bonsMatches.length > 0) {
      await notificarMatch({ usuarioId: item.usuarioId, itemTitulo: item.titulo, score: bonsMatches[0].score, io });
    }
...
const listarMatchesUsuario = async (req, res) => {
  try {
    const matches = await prisma.match.findMany({
      ...
      include: {
        itemPerdido: { include: { usuario: { select: { id: true, nome: true, avatar: true } } } },
        itemEncontrado: { include: { usuario: { select: { id: true, nome: true, avatar: true } } } }
      },
      orderBy: { score: 'desc' }
    });
...
/**
 * Confirmar que um match é correto
...
const confirmarMatch = async (req, res) => {
  try {
    ...
    await prisma.match.update({ where: { id: matchId }, data: { confirmado: true } });
    ...
    res.json({ sucesso: true, mensagem: 'Match confirmado! Os itens foram marcados como devolvidos.' });
  } catch (erro) {
